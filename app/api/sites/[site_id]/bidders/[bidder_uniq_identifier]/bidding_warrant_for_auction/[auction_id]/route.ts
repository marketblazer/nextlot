...
export async function GET(req: Request, context: { params: Promise<{ site_id: string; bidder_uniq_identifier: string; auction_id: string }> }) {
  const { site_id, bidder_uniq_identifier, auction_id } = await context.params
  const base = process.env.PROXY_TARGET || 'https://api-backend.nextlot.net/api/backend/v1'
  const targetUrl = `${base}/sites/${site_id}/bidders/${bidder_uniq_identifier}/bidding_warrant_for_auction/${auction_id}`
  const headers: Record<string, string> = { accept: 'application/json' }
  ...
  try {
    ...
    if (isJson) {
      ...
      if (Array.isArray(data)) {
        const normalized = data.map((item: any) => ({ ...item, site_id, bidder_uniq_identifier, auction_id }))
        return Response.json(normalized, { status: resp.status })
      }
      if (data && Array.isArray((data as any).items)) {
        const normalized = (data as any).items.map((item: any) => ({ ...item, site_id, bidder_uniq_identifier, auction_id }))
        return Response.json(normalized, { status: resp.status })
      }
      if (data && typeof data === 'object') {
        const normalized = { ...(data as any), site_id, bidder_uniq_identifier, auction_id }
        return Response.json(normalized, { status: resp.status })
      }
