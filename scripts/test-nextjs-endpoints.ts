// Test Next.js App Router endpoints directly
async function run() {
  ...
  console.log('🧪 Testing Next.js App Router Endpoints')
  console.log('=======================================')
  ...
  for (const endpoint of endpoints) {
    ...
    try {
      ...
      const resp = await fetch(`http://localhost:3000${endpoint.path}`, options)
      ...
      if (body && typeof body === 'object') {
        ...
        if (hasSuccess && hasData && hasMeta) {
          ...
          if (body.meta && body.meta.timestamp) {
